...
export function WeeklyPlannerCalendar() {
  ...
  const getItemColor = (item: SchedulableItem) => {
    ...
    if (!workstream) {
      ...
      return { color: "#1f2937", backgroundColor: "rgba(156, 163, 175, 0.25)", borderColor: "rgba(156, 163, 175, 0.5)" }
    }
                  ...
                  return (
                    ...
                    >
                      ...
                      {dayItems.map((item) => {
                        ...
                        const colorStyle = getItemColor(item)
                        const isDragging = draggedItem?.id === item.id
                        return (
                          <div
                            key={`${item.itemType}-${item.id}`}
                            className={`rounded-lg border cursor-move overflow-hidden group transition-all ${
          ...
          <div className="flex flex-wrap gap-2.5">
            {unscheduledItems.slice(0, 15).map((item) => {
              ...
              return (
                <div
                  key={`${item.itemType}-${item.id}`}
                  className="text-xs px-2.5 py-1.5 rounded-md border font-medium shadow-sm cursor-move hover:shadow-md transition-all flex items-center gap-1.5"
                  style={colorStyle}
                  draggable
                  onDragStart={(e) => handleDragStart(e, item)}
                  onDragEnd={handleDragEnd}
            ...
            {inProgressItems.length === 0 ? (
              ...
              inProgressItems.map((item) => {
                ...
                return (
                  <div
                    key={`${item.itemType}-${item.id}`}
                    className="text-xs px-3 py-2.5 rounded-lg border font-medium shadow-sm hover:shadow-md transition-all flex items-center justify-between gap-3"
                    style={colorStyle}
                  >
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <select
                        ...
                        onChange={(e) =>
                          handleStatusChange(item, e.target.value as "todo" | "in_progress" | "completed")
                        }
